...
### <a href="#section3"> 3. Exploratory Data Analysis and Initial Visualisation</a>
...
The dataset is based on studies that investigated the impact of the Black Summer bushfires and is designed to support future modelling and impact assessments. The dataset also has a 'sister' dataset of plant observations for the same purpose- see the <a href="#section6"> 4. challenge</a> for more!

The data consists of individual observations, recorded from 1634 onwards. Each row of the dataset represents a single sighting, recorded at a location, and includes information such as the invertebrates' scientific name, class, family, and year of sighting. There are 340059 observations. The data can be summarised as follows: 

|                 | Observations | Classes | Families | Scientific Names |
|-----------------|--------------|---------|----------|------------------|
| Invertebrates   | 340059       | 50      | 2171     | 50821            |
...
### 3. Exploratory Data Analysis and Initial Visualisation
...
The first step is to group our ```longitude``` and ```latitude``` columns into a single column named ```coords```, we do this using the ```nest``` function:
...
The code above also prints the first 6 rows of ```inverts_nest``` to show how the ```coords``` column contains [nested tibbles](https://tidyr.tidyverse.org/articles/nest.html). We also access the first of these tibbles using ```pluck```. -Try exploring this further by changing ```1``` to ```2```, ```3``` or ```4```!
...
```
<center><img src="{{ site.baseurl }}/Figures/aus_concave_hull.png" alt="Img"></center>
...
(worm_concave <- ggplot() + 
   ...
   theme_void())
```
<center><img src="{{ site.baseurl }}/Figures/earthworms.png" alt="Img"></center>
...
(spider_concave <- ggplot() + 
   ...
   theme_void())
```
<center><img src="{{ site.baseurl }}/Figures/spiders.png" alt="Img"></center>
...
<center><img src="{{ site.baseurl }}/Figures/blank_aus_hex.png" alt="Img"></center>
...
<center><img src="{{ site.baseurl }}/Figures/combined_plot.png" alt="Img"></center>
